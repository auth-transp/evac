...
sqr_distance(from, to, pathfinder::AnytimeDStar{D,true}) where {D} =
    sum(min.(abs.(from .- to), pathfinder.dims .- abs.(from .- to)) .^ 2)
sqr_distance(from, to, pathfinder::AnytimeDStar{D,false}) where {D} =
    sum((from .- to) .^ 2)
@inline function sqr_distance(from, to, pathfinder::AnytimeDStar{D,P}) where {D,P}
    s = pathfinder.dims
    delta = abs.(from .- to)
    sum(
...
"""
    find_continuous_path(pathfinder, from, to; max_iterations = nothing)
...
function find_continuous_path(
    ...
    discrete_path = find_path(pathfinder, discrete_from, discrete_to; max_iterations = max_iterations)
...
function Agents.plan_route!(
    agent::AbstractAgent,
    dest::Any,
    pathfinder::AnytimeDStar{D,P,M,Float64};
    max_iterations::Union{Int,Nothing} = nothing,
) where {D,P,M}
    path = find_continuous_path(pathfinder, agent.pos, dest; max_iterations = max_iterations)
...
function Agents.plan_best_route!(
    ...
    best_target = nothing
    for target in dests
        path = find_continuous_path(pathfinder, agent.pos, target; max_iterations = max_iterations)
...
"""
    move_along_route!(agent, model::ABM{<:ContinuousSpace{D}}, pathfinder::AnytimeDStar{D}, speed, dt = 1.0)
...
function random_walkable(model::ABM{<:ContinuousSpace{D}}, pathfinder::AnytimeDStar{D}) where {D}
...
walkable_cells_in_radius(pos, r, pathfinder::AnytimeDStar{D,false}) where {D} =
    Iterators.filter(
...
walkable_cells_in_radius(pos, r, pathfinder::AnytimeDStar{D,true}) where {D} =
    Iterators.map(
...
"""
    Pathfinding.random_walkable(pos, model::ABM{<:ContinuousSpace{D}}, pathfinder::AnytimeDStar{D}, r = 1.0)
