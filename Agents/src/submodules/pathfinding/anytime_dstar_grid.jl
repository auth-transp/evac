"""
    plan_route!(agent, dest, pathfinder::AnytimeDStar{D})
...
function Agents.plan_route!(
    ...
    path = find_path(pathfinder, agent.pos, dest; max_iterations = max_iterations)
    isnothing(path) && return
    pathfinder.agent_paths[Agents.getid(agent)] = path
end

"""
    plan_best_route!(agent, dests, pathfinder::AnytimeDStar{D}; kwargs...)
...
function Agents.plan_best_route!(
    agent::AbstractAgent,
    dests,
    pathfinder::AnytimeDStar{D,P,M,Int64};
    ...
    best_target = nothing
    for target in dests
        path = find_path(pathfinder, agent.pos, target; max_iterations = max_iterations)
...
"""
    move_along_route!(agent, model::ABM{<:GridSpace{D}}, pathfinder::AnytimeDStar{D})
...
"""
    Pathfinding.nearby_walkable(position, model::ABM{<:GridSpace{D}}, pathfinder::AnytimeDStar{D}, r = 1)
...
"""
nearby_walkable(position, model::ABM{<:GridSpace{D}}, pathfinder::AnytimeDStar{D}, r = 1) where {D} =
    Iterators.filter(x -> pathfinder.walkmap[x...] == 1, nearby_positions(position, model, r))


"""
    Pathfinding.random_walkable(model, pathfinder::AnytimeDStar{D})
Return a random position in the given `model` that is walkable as specified by the given
`pathfinder`.
"""
function random_walkable(model::ABM{<:GridSpace{D}}, pathfinder::AnytimeDStar{D}) where {D}
