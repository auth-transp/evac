sqr_distance(from, to, pathfinder::DStarLite{D,true}) where {D} =
    sum(min.(abs.(from .- to), pathfinder.dims .- abs.(from .- to)) .^ 2)
sqr_distance(from, to, pathfinder::DStarLite{D,false}) where {D} =
    sum((from .- to) .^ 2)
@inline function sqr_distance(from, to, pathfinder::DStarLite{D,P}) where {D,P}
    s = pathfinder.dims
    delta = abs.(from .- to)
    sum(
...
"""
    move_along_route!(agent, model::ABM{<:ContinuousSpace{D}}, pathfinder::DStarLite{D}, speed, dt = 1.0)
...
function random_walkable(model::ABM{<:ContinuousSpace{D}}, pathfinder::DStarLite{D}) where {D}
...
walkable_cells_in_radius(pos, r, pathfinder::DStarLite{D,false}) where {D} =
    Iterators.filter(
...
walkable_cells_in_radius(pos, r, pathfinder::DStarLite{D,true}) where {D} =
    Iterators.map(
...
"""
    Pathfinding.random_walkable(pos, model::ABM{<:ContinuousSpace{D}}, pathfinder::DStarLite{D}, r = 1.0)
