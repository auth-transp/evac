...
"""
    Pathfinding.AbsolutePenaltyMap(pmap::Array{Int,D} [, base_metric::CostMetric]) <: CostMetric{D}
Distance between two positions is the sum of the shortest distance between them and the
sum of the absolute values of the penalties at both positions.

Unlike [`Pathfinding.PenaltyMap`](@ref), which uses the absolute difference in penalty values,
`AbsolutePenaltyMap` adds the absolute value of each node's penalty, meaning the penalty
increases with the absolute value of the penalty at each node.
