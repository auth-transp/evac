...
%x(-1)=x(2)
%x(0)=x(3)
%Trying to add the ToxicLoad Function
%The graphs currently are only valid for a step size of 1 (h=1)
...
for i=4:length(t)
    ...
    Fmax=m*((v0-vo)/tau); %Motivation force at t=0 calculation
            ...
            TL=sum(TL>1)+TL(min(3,sum(TL>1)+1))*(1-(TL(3)>1)); %Function to extract only one highest value of TL - refer #010 OneNote
