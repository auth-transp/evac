...
begin   # Δημιουργία animation με trails & συλλογή CSV θέσης και toxicload
    ...
    video_file = "SCENARIO 2.2 (Contest) (no CM in PM) (DONE)/Simulation Results/SCENARIO_2.2_$(seed)_$(run_timestamp).mp4"
    ...
    csv_file = "SCENARIO 2.2 (Contest) (no CM in PM) (DONE)/Simulation Results/SCENARIO_2.2_$(seed)_$(run_timestamp).csv"
...
# --- Σταθερά bins ---
   ...
   y_inner = Observable(zeros(Float64, length(centers_inner)))  # (0,3) ανά 0.5
    ...
    out_file = joinpath(folder, "SCENARIO_2.2_hist_$(seed_str)_$(run_timestamp).mp4")
