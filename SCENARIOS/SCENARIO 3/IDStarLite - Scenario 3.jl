...
begin   # Αρχικοποίηση των παραμέτρων του μοντέλου
    ...
    n_agents = 25                                                        # Define the n_agents variable as 3
...
begin
    ...
    pathfinderPM = IDStarLite(space; walkmap = walkmap, cost_metric = cost_metric_obj)
    properties = (
...
@time begin   # Δημιουργία animation με trails & συλλογή CSV θέσης και toxicload
    ...
    ax  = Makie.Axis(fig[1,1];
               title  = "Evacuation with Toxic Trail (ID* Lite)",
               aspect = DataAspect())
    ...
    video_file = "SCENARIOS/SCENARIO 3/Simulation Results/IDStarLite_SCENARIO_3_$(n_agents)_$(seed)_$(cost_metric_str)_$(heuristic_code).mp4"
    csv_file   = "SCENARIOS/SCENARIO 3/Simulation Results/IDStarLite_SCENARIO_3_$(n_agents)_$(seed)_$(cost_metric_str)_$(heuristic_code).csv"  # added

    # keep a variable for the currently active map index
    current_map_idx = 1

    # --- INIT ID* Lite planners (one per exit) ---
    ...
    record(fig, video_file, 1:T; framerate=30) do frame
        ...
        if new_idx != current_map_idx
            ...
            # update cost metric IN-PLACE (no new pathfinder)
            ...
            # --- UPDATE ALL PLANNERS (ID* Lite runs HERE; skips when no changed cell was expanded) ---
...
begin
    ...
    csv_file = seed_str === nothing ? nothing : joinpath(folder, "IDStarLite_SCENARIO_3_$(n_agents)_$(seed_str)_$(cost_metric_str)_$(heuristic_code).csv")
    if csv_file === nothing || !isfile(csv_file)
        # αν δεν δοθεί seed, πάρε το πιο πρόσφατο *IDStarLite_SCENARIO_3_*.csv
        pattern = Regex("^IDStarLite_SCENARIO_3_.*_$(cost_metric_str)_$(heuristic_code)\\.csv\$")
        csvs = filter(f -> occursin(pattern, f), readdir(folder))
        @assert !isempty(csvs) "Δεν βρέθηκαν αρχεία *IDStarLite_SCENARIO_3_$(seed)*.csv στο $(folder)."
...
# --- Σταθερά bins ---
    ...
    ax  = CairoMakie.Axis(fig[1,1];
        title  = "Toxic Load (X) vs Agents Count (Y) — bins 0 & 3 separate, inner step 0.5 (ID* Lite)",
        xlabel = "Toxic load",
        ylabel = "Agents (count)",
    ...
    out_file = joinpath(folder, "IDStarLite_SCENARIO_3_hist__$(n_agents)_$(seed_str)_$(cost_metric_str)_$(heuristic_code).mp4")
