"""
    plan_route!(agent, dest, pathfinder::Dlite{D})
...
"""
    plan_best_route!(agent, dests, pathfinder::Dlite{D}; kwargs...)
...
"""
    move_along_route!(agent, model::ABM{<:GridSpace{D}}, pathfinder::Dlite{D})
...
"""
    Pathfinding.nearby_walkable(position, model::ABM{<:GridSpace{D}}, pathfinder::Dlite{D}, r = 1)
...
"""
nearby_walkable(position, model::ABM{<:GridSpace{D}}, pathfinder::Dlite{D}, r = 1) where {D} =
    Iterators.filter(x -> pathfinder.walkmap[x...] == 1, nearby_positions(position, model, r))


"""
    Pathfinding.random_walkable(model, pathfinder::Dlite{D})
Return a random position in the given `model` that is walkable as specified by the given
`pathfinder`.
"""
function random_walkable(model::ABM{<:GridSpace{D}}, pathfinder::Dlite{D}) where {D}
