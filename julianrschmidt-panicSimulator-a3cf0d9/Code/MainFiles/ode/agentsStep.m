...
%AgentsConc=diag(simulationObj.ConcentrationMap(round(agents(:,1)-0.5,0),round(agents(:,2)-0.5,0)));

for i=1:NAgent
    AgentsConc(1:NAgent,:)=Concentration;
end
TL=cTLV6(AgentsConc,dt,1,SymptomsSetup('H2S'),NAgent,agents(:,6:8));
agents(:,6:8)=TL;
...
if pressureBool
    [tVec, odeAgents, TE, ~, IE] = ode23(@(t,y)odeRhsWithPressure(t,y,radii,TL,columns,wallLines,exitCoord,settings, hObject),[t,t+dt],odeVec,odeOptions); %solve ODE with 'ode23'
