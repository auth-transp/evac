...
agentForceVecX = mass./tau .* (vDes*ex - agents(:,3));% .* (rand(NAgent, 1)*2+0.0); %acceleration in x-direction
agentForceVecY = mass./tau .* (vDes*ey - agents(:,4));% .* (rand(NAgent, 1)*2+0.0); %acceleration in x-direction
