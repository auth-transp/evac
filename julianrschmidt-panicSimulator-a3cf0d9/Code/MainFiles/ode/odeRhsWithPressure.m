function out = odeRhsWithPressure(~,odeVec,radii,TL,columns,wallLines, exitCoord, settings, hObject) % input: time vector dt and initial state vector odeVec
...
NAgent = size(odeVec,1) / 4;    %Its because of the way odeVec is defined, see agentsStep line 20
agents = [reshape(odeVec,NAgent,4),radii, TL];
...
% a1=0.8;
% b1=1.2;
% rnum1=(b1-a1).*rand(NAgent,1) + a1;
% 
% tau=tau*rnum1;

motivforcex=mass./(tau) .* (vDes*ex - agents(:,3));
motivforcey=mass./(tau) .* (vDes*ey - agents(:,4));
...
agentforcex=forceMatrixX;
agentforcey=forceMatrixY;
...
wallforcex=forceMatrixX;
wallforcey=forceMatrixY;
...
% TLx=cTLV5(ctf(agentX),t,1,SymptomsSetup('H2S'));        %Toxic Load Function
% TLx=sum(TLx>1)+TLx(min(3,sum(TLx>1)+1))*(1-(TLx(3)>1)); %Function to extract only one highest value of TL - refer #010 OneNote    
% 
% TLy=cTLV5(ctfy(agentY),t,1,SymptomsSetup('H2S'));       %Toxic Load Function
% TLy=sum(TLy>1)+TLy(min(3,sum(TLy>1)+1))*(1-(TLy(3)>1)); %Function to extract only one highest value of TL - refer #010 OneNote
 
% FmaxX=mass./tau .* (vDes*ex - agents(:,3));
% FmaxY=mass./tau .* (vDes*ex - agents(:,4));

ftX=zeros(1,NAgent);
ftY=zeros(1,NAgent);

% tau for symptoms, different than tau for motivational force

a2=1;
b2=1;
rnum2=(b2-a2).*rand(NAgent,1) + a2; %random number vector for symptau

symptau=rnum2;                      %randomised symptau between 0.8 & 1.2

for iAg=1:NAgent
   TLtotal=TL(iAg,:);
%             if TLtotal(1)>0.01 && TLtotal(2)<0.01 && TLtotal(3)<0.01 % Only level 1
%                 TLtotal=TLtotal(1);
%                 if TLtotal>1
%                     TLtotal=1;
%                 end
%             elseif TLtotal(1)>1 && TLtotal(2)>0.01 && TLtotal(3)<0.01 % Only level 2
%                 TLtotal=TLtotal(2)+1;
%                 if TLtotal>2
%                     TLtotal=2;
%                 end
%             elseif TLtotal(1)>1 && TLtotal(2)>1 && TLtotal(3)>0.01 % Only level 3
%                 TLtotal=TLtotal(3)+2;
%                 if TLtotal>3
%                     TLtotal=3;
%                 end
%             elseif TLtotal(1)>0.01 && TLtotal(2)>0.01 && TLtotal(3)<0.01 % Level 1&2
%                 TLtotal=TLtotal(1)+TLtotal(2);                               % Add level 1&2  
%                 if TLtotal>2
%                     TLtotal=2;
%                 end
%             elseif TLtotal(1)>0.01 && TLtotal(2)>0.01 && TLtotal(3)>0.01 % Level 1,2 & 3
%                 TLtotal=TLtotal(1)+TLtotal(2)+TLtotal(3);                % Add level 1,2&3  
%                 if TLtotal>3
%                     TLtotal=3;
%                 end
%             elseif TLtotal(1)<0.01 && TLtotal(2)<0.01 && TLtotal(3)<0.01 % below 1
%                         TLtotal=0;                                       % TL=0
%             elseif TLtotal(1)>1 && TLtotal(2)>0.01 && TLtotal(3)>0.01    % Level 2&3 
%                 TLtotal=1+TLtotal(2)+TLtotal(3);                % Add Level 1,2&3  
%                 if TLtotal>3
%                     TLtotal=3;
%                 end
%             end
            ...
            TLtotal=sum(TLtotal);
                ...
                if TLtotal==0    
                    v00=vDes;
                elseif TLtotal>0 && TLtotal<=1
                    v00=vDes;
                elseif TLtotal>1 && TLtotal<=2
                    v00=-1.151*log(TLtotal) + 1.4707;
                elseif TLtotal>2 && TLtotal<3
                    v00=-1.151*log(TLtotal) + 1.4707;
                elseif TLtotal>=3
                    v00=0;
                end
                
                 if (TLtotal>0) && (TLtotal<=1)
                    fsmell=TLtotal*30;
                elseif Concentration>140
                    fsmell=0;
                else
                    fsmell=0;
                end
...
%random number between 0.5 and 1.5 for v00
%             a3=0.5;
%             b3=1.5;
%             rnum3=(b3-a3).*rand(1,1) + a3; 
%             
%             v00=v00*rnum3;      %randomized v00 +/- 50%
             
            ftX(:,iAg)=((mass(iAg)/(symptau(iAg)))*(vDes-v00)*-ex(iAg))+(fsmell/mass(iAg));
            ftY(:,iAg)=((mass(iAg)/(symptau(iAg)))*(vDes-v00)*-ey(iAg))+(fsmell/mass(iAg));
...
agentForceVecX = agentForceVecX +ftX';
agentForceVecY = agentForceVecY +ftY';
