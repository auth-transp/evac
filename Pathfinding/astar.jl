...
"""
const Path{D,T} = MutableLinkedList{NTuple{D,T}} # MutableLinkedList ιδανικό όταν προσθαφαιρώ στοιχεία στα άκρα, ενώ το vector είναι πιο μπελαλίδικο

struct AStar{D,P,M,T,C<:CostMetric{D}} <: GridPathfinder{D,P,M} # D = ορισμός διαστάσεων, P = περιοδικότητα (true/false, μπορεί να λάβει και NTuple{D,Bool}, δες γραμμές 126-128), M = diagonal movement (true/false),T = τύπος συντεταγμένων (π.χ. Int, Float64), C<:CostMetric{D} = η 5η παράμετρος πρέπει να είναι υποτύπος του CostMetric{D} (δηλ. μια μετρική κόστους συμβατή με D διαστάσεις), π.χ. DirectDistance{D} ή PenaltyMap{D}, <: GridPathfinder{D,P,M} = ο AStar{…} είναι υποτύπος του αφηρημένου GridPathfinder{D,P,M}. Άρα κληρονομεί το «συμβόλαιο»/διεπαφή αυτού του υπερτύπου και μπορεί να χρησιμοποιηθεί όπου ζητείται ένας GridPathfinder με τα ίδια παραμετρικά D,P,M
    agent_paths::Dict{Int,Path{D,T}}    # λεξικό στο οποίο αποθηκεύονται τα επόμενα βήματα που θα ακολουθήσει ο agent.id 
    dims::NTuple{D,T}
...
Base.isempty(id::Int, pathfinder::AStar) =  # ελέγχει εάν ο agent με id έχει να διανύσει υπόλοιπη πορεία με το pathfinder του Astar
    !haskey(pathfinder.agent_paths, id) || isempty(pathfinder.agent_paths[id]) # μέσω του !haskey() ελέγχει εάν η βιβλιοθήκη agent_paths είναι συνδεδεμένη με τον agent.id, και μέσω του isempty() ελέγχει εάν το path που έχει αποθηκευτεί για τον agent.id είναι κενό
