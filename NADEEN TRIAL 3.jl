...
Person(id, pos, age, mass, toxicload) = AgentEscapes(id, pos, age, mass, toxicload)
heightmap_url = "Maps/test01.bmp"
...
function initialize_model(
    ...
    ag_range_x = (size(heightmap)[2]/2-250):(size(heightmap)[2]/2+250),
)
    heightmap = floor.(Int, Float32.(Gray.(load(heightmap_url))) * 39) .+ 1
...
function agent_step!(Person, model)
    # if is_stationary(AGENT, model.pathfinder)
        plan_route!(
            Person,
            random_walkable(Person.vel, model, model.pathfinder),
            model.pathfinder
        )
    # end
...
function static_preplot!(ac, model) 
    scatter!(ac, model.goals; color = (:red, 50), marker = 'o')
end
...
"EVAC_NEW_MOVEMENT.mp4", model, agent_step!, model_step!; plotkwargs, 
 figure = (resolution = size(heightmap),), frames = 600,
 framerate = 30,
 static_preplot!,
 original_colors,
