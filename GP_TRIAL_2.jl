...
begin
    ...
    seed = 111  ## seed for random number generator                     # Define the seed variable as 123
...
using CairoMakie  # Ensure you have a plotting library like CairoMakie loaded

fig = CairoMakie.Figure()
ax = CairoMakie.Axis(fig[1, 1];
    title = "Toxic Load Over Time",
    xlabel = "Step",
    ylabel = "Toxic Load"
)

for agent_id in unique(agent_records.id)
    agent_data = filter(row -> row.id == agent_id, eachrow(agent_records))
    CairoMakie.lines!(ax,
        getproperty.(agent_data, :step),
        getproperty.(agent_data, :toxicload),
